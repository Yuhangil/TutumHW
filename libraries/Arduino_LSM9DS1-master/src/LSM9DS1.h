...
class LSM9DS1Class {
  public:
    ...
    int begin(float XL_ODR = 50, float GS_ODR = 59.5 , int XL_FS = 2, int GS_FS = 500, int mode = 6, float MG_ODR = 40, int MG_FS = 16);       // 50Hz, 59.5Hz, 2g, 500 dps , Continuous Mode
    void SetXL(float XL_ODR, int XL_FS);
    void SetGS(float GS_ODR, int GS_FS);
    void SetFIFO(int mode);
    void end();
    void Reset();
