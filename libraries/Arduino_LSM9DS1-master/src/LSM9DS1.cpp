...
int LSM9DS1Class::begin(float XL_ODR, float GS_ODR, int XL_FS, int GS_FS, int mode, float MG_ODR, int MG_FS)
{
  _wire->begin();
  ...
  writeRegister(LSM9DS1_ADDRESS_M, LSM9DS1_CTRL_REG1_M, 0b11111000); // Temperature compensation enable, medium performance, 40 Hz
...
//writeRegister(LSM9DS1_ADDRESS_M, LSM9DS1_CTRL_REG2_M, 0x00); // 4 Gauss
  ...
  writeRegister(LSM9DS1_ADDRESS_M, LSM9DS1_CTRL_REG4_M, 0b00001100); // Z-axis operative mode medium performance
...
void LSM9DS1Class::SetXL(float XL_ODR, int XL_FS)
{
    ...
    if (XL_FS == 8)
        temp = temp | (3 << 3);
    writeRegister(LSM9DS1_ADDRESS, LSM9DS1_CTRL_REG6_XL, temp);

}
void LSM9DS1Class::SetGS(float GS_ODR, int GS_FS)
{
    int temp = 0;
    if (GS_ODR < 14.9)
        temp = 0;
    ...
    writeRegister(LSM9DS1_ADDRESS, LSM9DS1_CTRL_REG1_G, temp); // GS_ODR Hz, GS_FS scale
}
void LSM9DS1Class::SetFIFO(int mode)
{
    // 0 : Bypass, 1 : FIFO, 2 : Reserved, 3 : Continuous to FIFO, 4 : Bypass to Continuous, 6 : Continuous
    writeRegister(LSM9DS1_ADDRESS, 0x23, 0x02); // Enable FIFO
    writeRegister(LSM9DS1_ADDRESS, 0x2E, mode << 5);
}
void LSM9DS1Class::Reset()
{
    writeRegister(LSM9DS1_ADDRESS, 0x22, 0b00000101);
}
...
void LSM9DS1Class::measureODRcombined()
        //Combined measurement for faster startUp.
{ float x, y, z; 
